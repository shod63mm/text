...
<div class="max-w-[85rem] w-full mx-auto px-4 lg:px-8 mt-5">
    <div class="p-5 bg-blue-100 rounded-xl w-full">
      ...
      <h1 class="font-semibold text-xl text-blue-700"> Список работников</h1>
      <a href="{{ route('register') }}" class="bg-blue-700 text-white pb-1 px-3 rounded-full"> Добавить</a>
    </div>
            ...
            <div class="table border-collapse table-auto w-full divide-y divide-gray-200 dark:divide-gray-700 bg-blue-500">
              <div class="table-header-group">
                <div class="table-row">
                  ...
                  <div class="table-cell px-6 py-3 text-start text-xs font-medium text-white uppercase">Роль</div>
              ...
              <div class="table-row-group divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-slate-800">
                <div class="table-row">
                  ...
                  <div class="table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ $user->rolle }}</div>
                  <div class="table-cell px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                    <form id="deleteForm{{ $user->id }}" action="{{ route('users.delete', $user->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                            @if($user->email == 'sodmahmudov@gmail.com')
                            <button 
                            ...
                            dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">Delete
                            </button>
                            @else
                            <button 
                            type="button"
